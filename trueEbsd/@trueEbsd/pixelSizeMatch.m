function pixelSizeMatch(job, varargin)
% @trueEbsd method
% resize all job.interImgs images to the same pixel size
%
% Syntax
%   job.pixelSizeMatch(pixelsize)
%   job.pixelSizeMatch % default pixel size = smallest of job.imgList{:}.dx
%   job.pixelSizeMatch(0) % pixelsize = 0 - use default pixel size
%   job.pixelSizeMatch(pixelsize,'offsetMatch','centre','extentMatch','largest');
%   job.pixelSizeMatch(pixelsize,'offsetMatch','centre','extentMatch','smallest');
%   job.pixelSizeMatch(pixelsize,'offsetMatch','topLeft','extentMatch','largest');
%   job.pixelSizeMatch(pixelsize,'offsetMatch','topLeft','extentMatch','smallest');
% 
% 
% Input
%  job = @trueEbsd object
...
for n = 1:numel(job.imgList)
  ...
  if ~isempty(job.resizedList{n}.ebsd)
    ...
    %NOTE: don't try to use @EBSDsquare/interp because it only handles
    %indexed EBSD points when interpolating the map, this leads to e.g.
    %ebsd.bc disappearing from unindexed points
